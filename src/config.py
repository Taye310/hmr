...
flags.DEFINE_string('my_log_dir', 'logs', 'Where to save training models')
...
def prepare_dirs(config, prefix=['HMR']):
    ...
    if config.load_path:
        ...
        config.model_dir = osp.join(config.my_log_dir, save_name)

    for path in [config.my_log_dir, config.model_dir]:
